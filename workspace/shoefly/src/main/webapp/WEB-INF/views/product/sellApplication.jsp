	...
	<script>
		...
		function submitCheck(){
			$('#sellApplication_btn').click(function(){
				...
				//즉시 판매가 보다 낮은금액 입력시
				else if($('#price').val() <= $('#highPrice').val()){
					if(confirm('현재 신청하신 금액으로 즉시 판매하실 수 있습니다. \n즉시 판매하기 페이지로 이동하시겠습니까?')){
					...
					//판매신청서 작성
					//배송지테이블에 배송지삽입, 상품판매테이블에 상품삽입
					$('#f').attr('action', 'insertSellApplication.do');
					$('#f').serialize();
					$('#f').submit();
				}
			});
